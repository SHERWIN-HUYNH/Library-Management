...
public class ChiTietMuonTraServicelmpl {
	...
	public Date formatDate(Date inputDate) throws ParseException {
		SimpleDateFormat inputFormat = new SimpleDateFormat("EEE MMM dd HH:mm:ss z yyyy"); // Handle original format (if
																							// needed)
		SimpleDateFormat outputFormat = new SimpleDateFormat("dd-MM-yyyy");

		String formattedDateString = outputFormat.format(inputDate); // Format to String first
		Date formattedDate = inputFormat.parse(formattedDateString); // Parse the formatted string to create new Date
																		// object
